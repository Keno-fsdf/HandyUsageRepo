...
Random Forest reach $C{=}0.66$ and $C{=}0.69$ against the training
...
predictions sit, Google reaches $C{=}0.73$ and Linear $C{=}0.73$ versus
$0.50$ (TinyML) and $0.57$ (RF). At battery 50--75\,\%, Google and the
...
($C{=}0.74$) while all other methods drop into the $0.53\text{--}0.57$ band.
Bucketed on segment length, the 2--\SI{5}{\hour} bucket
($n{=}820$ common) is where Google attains its peak $C{=}0.96$ on
$y^{\text{extrap}}$, with all other methods at $C{=}0.87\text{--}0.91$.
...
TinyML reaches $C{=}0.66$ and Random Forest $C{=}0.69$, both above the
...
MAE ($\Delta{=}{+}0.07\,$h, $p_{\text{BH}}{=}0.55$) and on $C$
...
\begin{itemize}
\item \texttt{hotspot\_on} is positive in 7.1\,\% of samples; 99\,\%
of those positive samples come from the Pixel~8~Pro
(978 of 983 in the training split).
\item \texttt{wifi\_on} is positive in 8.1\,\% of samples; the
Pixel~8~Pro contributes the majority of WiFi-enabled samples
(1{,}475 of 1{,}643 across all devices), while the other three
devices stay below 1\,\% WiFi each.
...
\paragraph{Accuracy.}
TinyML and Random Forest reach $C{=}0.66$ and $C{=}0.69$ against the
...
from the simple linear baseline both on MAE ($p_{\text{BH}}{=}0.55$)
...
% Force entire bibliography (including the "References" heading)
% into the right-hand column of the last page
\newpage

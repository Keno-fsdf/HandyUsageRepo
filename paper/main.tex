...
lag steps, and typically below 5\,\% for 2-way and 3-way holdout splits. We replicate the
...
head. Total: 5{,}697 parameters. Trained with Adam~\cite{kingma2015adam}
...
converted to three TFLite~\cite{abadi2016tensorflow} variants (dynamic-range, float16, full-int8).
The full-int8 variant is the deployed artefact in the
companion app.

\item \textbf{Random Forest sanity model.} A scikit-learn~\cite{pedregosa2011scikit}
...
We attach 95\% bootstrap~\cite{efron1979bootstrap} confidence intervals to MAE, RMSE, ME and $C$
(1{,}000 resamples; 250 for $C$, which is $O(n^2)$). Pairwise method
...
We report both raw $p$-values and
Benjamini--Hochberg~\cite{benjamini1995controlling} FDR-adjusted
...
\texttt{screen\_on}             & $-0.34$ & $-0.25$ & 0.15 \\
...
(iv)~\textbf{IoT and embedded battery-powered
devices}~\cite{banbury2021mlperf,alajlan2022tinyml} (sensor nodes,
environmental monitors, asset trackers)
...
Concordance index as an evaluation metric, since it remains
